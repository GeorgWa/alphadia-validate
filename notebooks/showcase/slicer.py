...
class SpectrumSlicer:
    def __init__(self, spectral_library_flat: SpecLibFlat, precursor_df: pd.DataFrame, dia_data: Thermo):
        self.spectral_library_flat = spectral_library_flat
        self.precursor_df = precursor_df
        self.dia_data = dia_data

    def get_by_hash(self, selected_hash: int) -> tuple[np.ndarray, pd.DataFrame, pd.DataFrame]:
        ...
        speclib_entry, mz_library, intensity_library = get_flat_library_entry_by_hash(self.spectral_library_flat, selected_hash)
        precursor_entry = self.precursor_df[self.precursor_df['mod_seq_charge_hash'] == selected_hash].iloc[0]
