...
RAW_DATA_URL_LIST = "https://datashare.biochem.mpg.de/s/339jg5HtGrwLwDN/download?files=20231017_OA2_TiHe_ADIAMA_HeLa_200ng_Evo011_21min_F-40_07.raw",
...
def prepare_data(main_folder: Path) -> tuple[pd.DataFrame, SpecLibBase, SpecLibFlat, Thermo]:
...
def _download_data(main_folder: Path) -> tuple[pd.DataFrame, str, str]:
    ...
    for folder in [data_folder, output_folder]:
        if not os.path.exists(folder):
            os.makedirs(folder)
    raw_file_paths = [DataShareDownloader(url, str(data_folder)).download() for url in RAW_DATA_URL_LIST]
    precursors_tsv_path = DataShareDownloader(PRECURSORS_TSV_URL, str(output_folder)).download()
