...
def get_isoform_id(mods, mod_sites, translate_mod_dict: dict = None, ignore_mods={}):
    """
    ...
    if mods:
        ...
        isoform_id = ";".join(
            [f"{mod}{site}" for site, mod in zip(var_mod_sites, var_mods)]
        )
...
def fragment_isoform_annotation(precursor_entry, fixed_mods={"Carbamidomethyl@C"}):
    ...
    var_mod_count = [
        mod_counter[mod] for mod in mod_counter.keys() if mod not in fixed_mods
    ]
    ...
    potential_fragements = [
        f"{type}_z{z}" for type in ["b", "y"] for z in range(1, charge + 1)
    ]
    ...
    for _, e in flatlib.precursor_df.iterrows():
        if e["mod_sites"] != "":
            fragment_table.loc[
                e["flat_frag_start_idx"] : e["flat_frag_stop_idx"], "isoform"
...
def isoform_attention_plot(
    spectral_library_flat, precursor_df, dia_data, selected_hash, match_tolerance=7e-6
):
    ...
    library_spec["intensity"] = (
        library_spec["intensity"] / library_spec["intensity"].max()
    )
